...
namespace UnityEditor.Tilemaps
{
    ...
    ///     Use this as an example to create brushes which store specific data per brush and to make brushes which randomize
    ///     behaviour.
    /// </summary>
    [HelpURL(
    ...
    public class RandomBrush : GridBrush
    {
        ...
        public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position)
        {
            if (randomTileChangeDataSets != null && randomTileChangeDataSets.Length > 0)
            {
                ...
                {
                    var randomTileChangeDataSet =
                        randomTileChangeDataSets[(int)(randomTileChangeDataSets.Length * Random.value)];
        ...
        public override void Pick(GridLayout gridLayout, GameObject brushTarget, BoundsInt bounds, Vector3Int pickStart)
        {
            ...
            foreach (var startLocation in new SizeEnumerator(bounds.min, bounds.max, randomTileSetSize))
            {
                ...
                randomTileChangeDataSets[i].randomTileChangeData =
                    new TileChangeData[randomTileSetSize.x * randomTileSetSize.y * randomTileSetSize.z];
                var randomBounds = new BoundsInt(startLocation, randomTileSetSize);
                var j = 0;
                foreach (var pos in randomBounds.allPositionsWithin)
                {
                    ...
                    randomTileChangeDataSets[i].randomTileChangeData[j].transform =
                        inBounds ? tilemap.GetTransformMatrix(pos) : Matrix4x4.identity;
                    randomTileChangeDataSets[i].randomTileChangeData[j].color =
                        inBounds ? tilemap.GetColor(pos) : Color.white;
                    j++;
                }
    ...
    public class RandomBrushEditor : GridBrushEditor
    {
        private static readonly string iconPath =
            "Packages/com.unity.2d.tilemap.extras/Editor/Brushes/RandomBrush/RandomBrush.png";
        ...
        public override void PaintPreview(GridLayout grid, GameObject brushTarget, Vector3Int position)
        {
            if (randomBrush.randomTileChangeDataSets != null && randomBrush.randomTileChangeDataSets.Length > 0)
            {
                ...
                {
                    var randomTileChangeDataSet =
                        randomBrush.randomTileChangeDataSets[
                            (int)(randomBrush.randomTileChangeDataSets.Length * Random.value)];
                    ...
                    foreach (var pos in randomBounds.allPositionsWithin)
                    {
                        ...
                        tilemap.SetEditorPreviewTransformMatrix(pos,
                            randomTileChangeDataSet.randomTileChangeData[j].transform);
        ...
        public override Texture2D RenderStaticPreview(string assetPath, Object[] subAssets, int width, int height)
        {
            ...
            var previewInstance =
                new GameObject("Brush Preview", typeof(Grid), typeof(Tilemap), typeof(TilemapRenderer));
