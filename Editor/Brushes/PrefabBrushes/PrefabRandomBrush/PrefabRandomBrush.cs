...
{
    /// <summary>
    ///     This Brush instances and places a randomly selected Prefabs onto the targeted location and parents the instanced
    ...
    public class PrefabRandomBrush : BasePrefabBrush
    {
        ...
        ///     Otherwise, erases only GameObjects that are created from owned Prefab in a given position within the selected
        ///     layers with Erasing.
        /// </summary>
        ...
        public override void Paint(GridLayout grid, GameObject brushTarget, Vector3Int position)
        {
            ...
            if (brushTarget == null || brushTarget.layer == 31) return;
            ...
            {
                return m_Prefabs.Any(prefab =>
                    PrefabUtility.GetCorrespondingObjectFromSource(objectInCell) == prefab);
            });

            if (!existPrefabObjectInCell)
            {
                var index = Mathf.Clamp(
                    Mathf.FloorToInt(GetPerlinValue(position, m_PerlinScale, k_PerlinOffset) * m_Prefabs.Length), 0,
                    m_Prefabs.Length - 1);
        ...
        ///     If "Erase Any Objects" is false, erases only GameObjects that are created from owned Prefab in a given position
        ///     within the selected layers.
        ...
        public override void Erase(GridLayout grid, GameObject brushTarget, Vector3Int position)
        {
            ...
            foreach (var prefab in m_Prefabs)
                if (objectInCell != null && (m_EraseAnyObjects ||
                                             PrefabUtility.GetCorrespondingObjectFromSource(objectInCell) == prefab))
        ...
        public class PrefabRandomBrushEditor : BasePrefabBrushEditor
        {
            ...
            public override void OnPaintInspectorGUI()
            {
                ...
                prefabRandomBrush.m_PerlinScale =
                    EditorGUILayout.Slider("Perlin Scale", prefabRandomBrush.m_PerlinScale, 0.001f, 0.999f);
                EditorGUILayout.PropertyField(m_Prefabs, true);
                prefabRandomBrush.m_EraseAnyObjects =
                    EditorGUILayout.Toggle("Erase Any Objects", prefabRandomBrush.m_EraseAnyObjects);
