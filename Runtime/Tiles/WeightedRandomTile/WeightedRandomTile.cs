...
namespace UnityEngine.Tilemaps
{
    ...
    /// <summary>
    ///     Weighted Random Tiles are tiles which randomly pick a sprite from a given list of sprites and a target location,
    ///     and displays that sprite.
    ///     The sprites can be weighted with a value to change its probability of appearing. The Sprite displayed for the Tile
    ///     is randomized based on its location and will be fixed for that particular location.
    /// </summary>
    [Serializable]
    ...
    public class WeightedRandomTileEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var count = EditorGUILayout.DelayedIntField("Number of Sprites",
                Tile.Sprites != null ? Tile.Sprites.Length : 0);
            if (count < 0)
                count = 0;

            if (Tile.Sprites == null || Tile.Sprites.Length != count) Array.Resize(ref Tile.Sprites, count);
